...
class PortScanner():
    ...
    def scan_in_a_new_thread(self):
        ...
        thread = Thread(target=self.start_scan, args=(url, start_port, end_port))
        thread.start()
    ...
    def create_gui(self):
        ...
        Label(self.root, text='Start Port :').grid(row="2", column="1", sticky=W)
        ...
        Button(self.root, text='Scan', command=self.on_scan_button_clicked).grid(row="4", column="2", sticky=E)
        Button(self.root, text='Stop', command=self.on_stop_button_clicked).grid(row="4", column="2", sticky=W)
         
        Label(self.root, text='Scan Result :').grid(row="5", column="1", sticky=W)
